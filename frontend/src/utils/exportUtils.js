  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Read as array of arrays to handle merged cells and title
        const aoa = XLSX.utils.sheet_to_json(ws, { header: 1, defval: "" });
        console.log("📄 Raw Excel data (aoa):", aoa);

        // aoa[0] = title row, aoa[1] = header row, aoa[2+] = data rows
        if (aoa.length < 3) {
          reject(new Error("File Excel format tidak valid"));
          return;
        }

        const headerRow = aoa[1]; // Baris 2 (index 1) adalah header
        const dataRows = aoa.slice(2); // Baris 3+ (index 2+) adalah data

        // Convert to object array
        const jsonData = dataRows.map((row, idx) => {
          const obj = {};
...
export function downloadTemplate(columns, filename = "template") {
  ...
  XLSX.utils.sheet_add_aoa(
    ws,
    [[`IMPORT TEMPLATE: ${filename.toUpperCase()}`]],
    { origin: "A1" },
  );
  if (!ws["!merges"]) ws["!merges"] = [];
  ws["!merges"].push({ s: { r: 0, c: 0 }, e: { r: 0, c: columns.length - 1 } });
